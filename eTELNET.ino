...
// Modified by WOEPER 2025 for OTA extension and Led status 
...
WiFiEventHandler	wifiAuthModeChangedHandler;
...
static void applySerialSettings()
{
    Serial.flush();
    delay(100);
...
static void readString(char* buffer, int buflen, bool password)
{
    int len = 0;
    while (true)
    {
...
static void GetWiFiData(const char* msg)
{
    clearSerialBuffer();
...
void setup()
{
    ...
    pinMode(WIF_PIN, OUTPUT); 
    pinMode(ACT_PIN, OUTPUT);
    
    digitalWrite(LED_PIN, HIGH);

    digitalWrite(WIF_PIN, LOW);
    ...
    while (*WifiData.ssid != 0 && (WiFi.status() != WL_CONNECTED))
    {
        ...
        sprintf(Hostname, "eTELSER_%X", ESP.getChipId());
        MDNS.begin(Hostname);
        hMDNSService = MDNS.addService(0, "http", "tcp", 80);
        MDNS.addServiceTxt(F("http"), F("tcp"), F("Manufacturer"), F("WOEPER"));
        MDNS.addServiceTxt(F("http"), F("tcp"), F("MAC address"), WiFi.macAddress());
        MDNS.addServiceTxt(F("http"), F("tcp"), F("SDK version"), String(ESP.getSdkVersion()));
        MDNS.addServiceTxt(F("http"), F("tcp"), F("Reset reason"), ESP.getResetReason().c_str());
        MDNS.addServiceTxt(F("http"), F("tcp"), F("Core version"), ESP.getCoreVersion().c_str());

        wifiConnectHandler = WiFi.onStationModeGotIP(onWifiConnect);
        wifiDisconnectHandler = WiFi.onStationModeDisconnected(onWifiDisconnect);
        wifiAuthModeChangedHandler = WiFi.onStationModeAuthModeChanged(onWifiAuthenticationChanged);
...
static void resetTelnetState(struct TelnetStateStruct& s)
{
    s.cmdLen = 0;
    s.sendBinary = false;
...
static void printModemResult(byte code)
{
    if (!modemQuiet)
    {
        if (modemVerbose)
...
static byte getCmdParam(char* cmd, int& ptr)
{
    byte res = 0;
...
static bool handleTelnetProtocol(uint8_t b, WiFiClient& client, struct TelnetStateStruct& state, bool isServer)
{
...
static void handleModemCommand()
{
            ...
            while (modemClient.available() && Serial.availableForWrite() && millis() - t < 100)
            {
                ...
                if (!handleTelnetProtocol(b, modemClient, modemTelnetState, false)) {
                    ...
                    while (digitalRead(CTS_PIN) == LOW) {
                        // CTS is low => modem is not ready to receive data
                        delay(100);
...
static void onWifiAuthenticationChanged(const WiFiEventStationModeAuthModeChanged& event) {
    digitalWrite(WIF_PIN, LOW);
    WiFi.reconnect();
}
